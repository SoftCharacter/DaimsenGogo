...
仓库内置 5 个 DG 分析示例主题：华为昇腾芯片供应链、存储/HBM产业供应链、CPO/1.6T光模块供应链、英伟达AI数据中心电源链、张雪机车供应链。下载后即使不配置大模型和 Tavily API Key，也可以直接启动项目并进入“供应链看板”查看展示形态；只有新建 DG 分析和“智能解盘”等大模型能力需要配置 API。
