...
6. 递归补搜阶段在已确证候选不足目标数量时继续查漏，重点搜索公告、年报、互动易、合作关系和投资关系等差异化线索，并对新补候选补齐同样的 `business_profile` 后再进入评分。
...
| 公司业务画像 | `ak.stock_zyjs_ths(symbol="000725")` / `ak.stock_profile_cninfo(symbol="000725")` |
...
6. DG 分析评分前的业务画像：调用 `fetch_company_business_info_sync(code, task_id)`，优先从 `stock_zyjs_ths` 和 `stock_profile_cninfo` 清洗出 `主营业务`、`产品名称`、`经营范围` 三个字段，组装为 `business_profile` 传给业务确证和递归补搜评分模型。
...
- 当前正式评分链路只使用 AkShare 业务画像，雪球 Cookie 方案暂未接入生产兜底。
...
3. 在同页填写 Tavily API Key；DG 分析必须先完成 `web_search` 配置。
