...
3. `plan_execute_react_loop` 进入固定 SOP：链路编排、线索捕获、业务确证、递归补搜、规则校准、主题成图。
4. 线索捕获阶段由大模型生成严格 JSON 搜索计划，后端调用 `web_search` 获取公开网页线索，并与本地 A 股股票基础列表比对，得到候选池。
5. 业务确证阶段对每只候选补充 `get_company_info` 基础信息，并继续搜索“公司 + 主题”的业务关系线索；LLM 根据统一材料包输出 `relation_score`、关系类型、业务摘要和依据。
6. 递归补搜阶段在已确证候选不足目标数量时继续查漏，重点搜索公告、年报、互动易、合作关系和投资关系等差异化线索。
...
10. 前端通过 SSE 接收任务事件，展示推理过程、候选股票、确认节点和最终主题结构。

这条链路的重点是：大模型负责搜索规划、语义确证和结构化推理，本地工具负责 A 股候选比对、公司信息补充、候选池一致性校准和数据缓存，最终结果以本地 JSON 主题文件沉淀，便于后续看板复用和二次开发。
...
- `web_search` 未启用或未配置密钥时，线索捕获会回退到本地 `search_stocks` 兜底，业务确证和递归补搜会跳过网页线索。
