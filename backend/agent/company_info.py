"""
公司信息查询工具模块
通过AkShare获取上市公司业务画像，供ReAct Agent工具调用。
"""
import logging
import time
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            if attempt < _MAX_RETRIES:
                logger.warning(
                    "akshare公司业务画像第%d次失败 [%s]: %s, %.1f秒后重试",
                    attempt, pure_code, exc, _RETRY_DELAY,
                )
                time.sleep(_RETRY_DELAY)

    logger.warning("akshare公司业务画像失败 [%s]: %s", pure_code, last_err)
    raise RuntimeError(f"akshare公司业务画像获取失败: {last_err}")
