...
def _build_quote_from_daily(code: str, task_id: str | None = None) -> StockQuote:
    """用最近两根日K生成兜底行情，避免实时行情接口不可用"""
    num = extract_numeric_code(code)
    points = fetch_recent_daily_kline_sync(num, task_id=task_id)
    latest = points[-1] if points else None
    prev = points[-2] if len(points) >= 2 else None
    info = fetch_company_info_sync(num, task_id=task_id)

    current = latest.close if latest else 0.0
    prev_close = prev.close if prev else current
    change = current - prev_close
    change_percent = (change / prev_close * 100) if prev_close else 0.0

    return StockQuote(
        code=format_stock_code(num),
        name=_company_name_from_info(info),
        current_price=round(current, 2),
        prev_close=round(prev_close, 2),
        open_price=round(latest.open if latest else 0.0, 2),
        high=round(latest.high if latest else 0.0, 2),
        low=round(latest.low if latest else 0.0, 2),
        change=round(change, 2),
        change_percent=round(change_percent, 2),
        volume=latest.volume if latest else 0.0,
        volume_display=format_volume(latest.volume if latest else 0.0),
...
def get_stock_list(task_id: str | None = None) -> list[dict]:
    ...
    """
    cached = _read_json_cache(_stock_list_cache_path(task_id), [])
...
def fetch_quote_xq_sync(code: str, task_id: str | None = None) -> StockQuote | None:
    """
    使用雪球个股行情接口获取指定股票实时行情。
    """
    formatted_code = format_stock_code(extract_numeric_code(code))
    quote_data = _fetch_xq_quote(_to_xq_symbol(formatted_code))
    if not quote_data:
        cached = _read_json_cache(_quote_cache_path(task_id), {}).get(formatted_code)
        return StockQuote(**cached) if cached else None
...
def _write_kline_cache(code: str, points: list[KLinePoint], task_id: str | None = None) -> None:
    """写入任务级K线缓存。"""
    cached_klines = _read_json_cache(_kline_cache_path(task_id), {})
    cached_klines[format_stock_code(extract_numeric_code(code))] = [point.model_dump() for point in points]
    _write_json_cache(_kline_cache_path(task_id), cached_klines)


def _fetch_recent_daily_kline_xq_sync(code: str, count: int = _MONTH_TRADING_DAYS) -> list[KLinePoint]:
    """使用雪球原生日K接口获取近一个月K线。"""
    symbol = _to_xq_symbol(code)
    payload = _fetch_xq_json_sync(
        symbol,
    ...
    if missing:
        raise RuntimeError(f"雪球K线响应缺少必要字段: {missing}, columns={columns}")
    volume_index = column_index.get("volume", column_index.get("amount"))

    points: list[KLinePoint] = []
    for item in items[-abs(int(count)):]:
        max_index = max(column_index[name] for name in required)
        if volume_index is not None:
            max_index = max(max_index, volume_index)
        if len(item) <= max_index:
            continue
        timestamp = int(item[column_index["timestamp"]])
        points.append(KLinePoint(
            date=datetime.fromtimestamp(timestamp / 1000).strftime("%Y-%m-%d"),
            open=_safe_float(item[column_index["open"]]),
            high=_safe_float(item[column_index["high"]]),
            low=_safe_float(item[column_index["low"]]),
            close=_safe_float(item[column_index["close"]]),
            volume=_safe_float(item[volume_index]) if volume_index is not None else 0.0,
        ))
    if not points:
        raise RuntimeError("雪球K线响应没有可用数据行")
    return points


def _fetch_recent_daily_kline_akshare_sync(code: str, task_id: str | None = None) -> list[KLinePoint]:
    """使用AkShare腾讯接口获取近一个月K线。"""
    import akshare as ak
    ...
    try:
        ...
        cached_klines = _read_json_cache(_kline_cache_path(task_id), {})
        return [KLinePoint(**item) for item in cached_klines.get(format_stock_code(num), [])]

    points: list[KLinePoint] = []
    for _, row in df.tail(_MONTH_TRADING_DAYS).iterrows():
        points.append(KLinePoint(
    ...
    _write_kline_cache(num, points, task_id)
    return points


def fetch_recent_daily_kline_sync(code: str, task_id: str | None = None) -> list[KLinePoint]:
    """优先使用雪球获取近一个月日K，失败时回退AkShare。"""
    num = extract_numeric_code(code)
    try:
        points = _fetch_recent_daily_kline_xq_sync(num, _MONTH_TRADING_DAYS)
        _write_kline_cache(num, points, task_id)
        return points
    except Exception as exc:
        logger.warning("雪球获取近一个月日K失败 [%s]，回退AkShare: %s", code, exc)
    return _fetch_recent_daily_kline_akshare_sync(num, task_id=task_id)
