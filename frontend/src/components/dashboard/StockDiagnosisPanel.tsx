...
export default function StockDiagnosisPanel({
  ...
  const llmStatusText =
    diagnosis?.llm_status === 'error' || diagnosis?.llm_status === 'missing_config'
      ? '大模型调用失败，请配置 API'
      : ''
  const dataErrorEntries = Object.entries(diagnosis?.data_errors || {})
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.6fr) minmax(0, 1fr)', flex: 1, minHeight: 0, overflow: 'hidden' }}>
            ...
            <div style={{ minWidth: 0, overflowY: 'auto', padding: '20px 22px' }}>
              ...
              {dataErrorEntries.length > 0 && (
                <div style={{ background: 'rgba(255, 91, 91, 0.10)', border: '1px solid rgba(255, 91, 91, 0.30)', borderRadius: 'var(--r-sm)', padding: '10px 12px', marginBottom: 14 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--up)', letterSpacing: '0.06em', marginBottom: 5 }}>接口取数异常</div>
                  <div style={{ fontSize: 12, color: 'var(--text-dim)', lineHeight: 1.55 }}>
                    {dataErrorEntries.map(([label, message]) => (
                      <div key={label}>{label}: {message || '取数失败，请检查接口'}</div>
                    ))}
                  </div>
                </div>
              )}
