    ...
    >
      ...
      <div style={{ padding: '14px 16px 8px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
          <span
            className="card-name"
            style={{ minWidth: 0, flex: 1, fontWeight: 600, fontSize: 15.5, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
          >
            {stock.name}
          </span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, flex: 'none', marginTop: 3 }}>
            <span className="ticker" style={{ fontSize: 11, color: 'var(--text-faint)' }}>
              {stock.code}
            </span>
            <span className="num" style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent-bright)' }}>
              关联 {stock.percentage}
            </span>
          </span>
        </div>
